...
export default class Events {
  ...
  static onGameAdded(callback) {
    document.addEventListener(events.gameAdded, (event) => {
      return callback(event.detail);
    });
  }
  static dispatchGameAdded(data) {
    const event = new CustomEvent(events.gameAdded, {
      detail: data,
    });
    document.dispatchEvent(event);
  }

  static onGameRemoved(callback) {
    document.addEventListener(events.gameRemoved, (event) => {
      return callback(event.detail);
    });
  }

  static dispatchEventGameRemoved(data) {
    const event = new CustomEvent(events.gameRemoved, {
      detail: data,
    });
    document.dispatchEvent(event);
  }
